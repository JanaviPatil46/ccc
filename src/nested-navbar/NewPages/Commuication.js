  ...
  const saveChat = () => {
    ...
    fetch(`${CHATTOCLIENT_API}/chats/chatsaccountwise`, requestOptions)
      ...
      .then((result) => {
        ...
        // Clear all input fields except "To"
        // setSelectInvoiceTemp(null); // Clear template
        setTemplateName(""); // Clear template name
        setInputText(""); // Clear subject input
        setSelectedShortcut(""); // Clear shortcut
        // setAbsoluteDate(false); // Reset reminders toggle
        setDaysuntilNextReminder(""); // Clear days until next reminder
        setNoOfReminder(""); // Clear number of reminders
